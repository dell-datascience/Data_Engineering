...
def upload() -> None:
    """
    downloads url dataset, loads dataset, uploads dataset into postgresql docker from docker-compose.yaml file
    ...
    engine = create_engine(
        f"postgresql://{user}:{password}@{host}:{port}/{databaseName}"
    )
    ...
    try:
        for data in data_iter:
            ...
            print(
                f"\n **** Inserted {len(data)} chunk data ... took %.3f"
                % (time() - start_time)
            )
